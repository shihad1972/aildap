...
init_kv_s(ailsa_kv_s **kv)
{
	...
	if (!(data->name = calloc(DOMAIN, sizeof(char))))
		error(MALLOC, errno, "kv->name in init_kv_s");
...
put_kv_key(ailsa_kv_s *kv, const char *name)
{
        ...
        if (!(kv->name))
                return NODATA;
        if (snprintf(kv->name, DOMAIN, "%s", name) >= DOMAIN) {
                rep_truncate("name", DOMAIN);
                retval = TRUNC;
        }
        return retval;
}
