...
get_ldif_pass_hash(char *pass)
{
	...
	char *npass, salt[6], type[] = "sha", *tpass;
	guchar *out;
        EVP_MD_CTX *msg;
        const EVP_MD *md;
        unsigned int md_len, c;
	size_t len = strlen(pass) + 6;

	if (!(out = malloc(26)))
		rep_err("out in get_ldif_pass_hash");
	if (!(tpass = malloc(len)))
		rep_err("tpass in get_ldif_pass_hash");
	if ((read(rd, &salt, 6)) != 6) {
		close(rd);
...
/*	EVP_DigestUpdate(msg, pass, strlen(pass));
	...
	EVP_DigestUpdate(msg, tpass, len);
